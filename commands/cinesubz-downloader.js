...
module.exports = {
    ...
    handler: async ({ socket, msg, sender, command, args, reply }) => {
        const botName = "👸 𝗤𝗨𝗘𝗘𝗡 𝗡𝗘𝗟𝗨𝗠𝗜 𝗠𝗗 👸";
        const CZ_API = "https://cz-dnuz.vercel.app";
        const OLD_API = "https://cinesubz-api-cnw.vercel.app/api";
        const metaQuote = {
            key: { remoteJid: "status@broadcast", participant: "0@s.whatsapp.net", fromMe: false, id: "NELUMI_AI_CZ" },
        ...
        if (command === "cz" || command === "cinesubz") {
            ...
            if (!query) {
                return await reply("🧚‍♀️ *කරුණාකර Movie එකේ නම ලබා දෙන්න!*\n_උදා: .cz batman_");
            }
                ...
                const listener = async ({ messages }) => {
                    try {
                        ...
                        if (isNaN(selectedIndex) || selectedIndex < 0 || selectedIndex >= topResults.length) {
                            return await socket.sendMessage(sender, { text: "🚫 *වැරදි අංකයක්! කරුණාකර නිවැරදි අංකය ලබා දෙන්න.*" }, { quoted: replyMsg });
                        }
                        ...
                        await socket.sendMessage(sender, { text: `✨ *${selectedMovie.title}* සඳහා Download Links සකසමින් පවතී... 🧚🏼‍♀️` }, { quoted: replyMsg });
                        ...
                        const captionText = `*✦─────[ 🎬 𝗤𝗨𝗘𝗘𝗡 𝗡𝗘𝗟𝗨𝗠𝗜 𝗠𝗢𝗩𝗜𝗘𝗦 🎬 ]─────✦*\n\n🌸 *NAME:* ${movieTitle}\n📅 *DATE:* ${selectedMovie.date || 'N/A'}\n🎭 *QULITY:* ${selectedMovie.genres || 'N/A'}\n⭐ *IMDB :* ${selectedMovie.imdb || 'N/A'}\n⏱ *TIME:* ${selectedMovie.runtime || 'N/A'}\n\n> 🧚🏼‍♀️ *ඔබට අවශ්‍ය Quality එක පහලින් තෝරන්න* ⬇️`;

                        await socket.sendMessage(sender, {
                            image: { url: selectedMovie.img },
                            caption: captionText,
                            footer: '👸 𝗤𝗨𝗘𝗘𝗡 𝗡𝗘𝗟𝗨𝗠𝗜 𝗠𝗗 👸',
                            buttons: buttons,
                            headerType: 4
                        }, { quoted: replyMsg });
        ...
        // ==========================================
            ...
            try {
                await socket.sendMessage(sender, { react: { text: "📥", key: msg.key } });
                await socket.sendMessage(sender, { text: `🧚🏼‍♀️ *${title} (${quality}) Download වෙමින් පවතී...*\n_මොහොතක් රැඳී සිටින්න..._` }, { quoted: metaQuote });
                ...
                if (downloadSuccess) {
                    ...
                    await socket.sendMessage(sender, { react: { text: "❌", key: msg.key } });
                    await reply("💔 *Download Failed! සර්වර් එකේ ලින්ක් එක කල් ඉකුත් වී ඇත.*");
                }

            } catch (e) {
                console.error("Cinesubz DL Error:", e.message);
                await socket.sendMessage(sender, { react: { text: "❌", key: msg.key } });
                await reply("💔 *Download කරගැනීමට නොහැකි විය!*");
            }
        }
    }
};
