...
tgBot.on('message', async (msg) => {
    ...
    if (text === '/start') {
        await tgBot.sendMessage(chatId, "𝗪𝗘𝗟𝗖𝗢𝗠𝗘 𝗧𝗢 𝗘𝗩𝗔 𝗠𝗜𝗡𝗜-𝗕𝗢𝗧\n\n𝗘𝗡𝗧𝗘𝗥 𝗬𝗢𝗨𝗥 𝗪𝗛𝗔𝗧𝗦𝗔𝗣𝗣 𝗡𝗨𝗠𝗕𝗘𝗥\n(Example: 94703945265)");
        return;
    }
...
async initialize(pairingNumber = null) {
    ...
    try {
        ...
        // ============================================
        // 🔥 CHANGE 3: LOAD SESSION FROM MONGODB
        // ============================================
        let savedState = await database.loadSession(this.userId);
        let state;
        let saveCreds;
        
        if (savedState) {
            // Session එක MongoDB එකෙන් හම්බුනා
            state = {
            ...
            // Session එක නැහැ, අලුතින් හදන්න
            const { state: newState, saveCreds: newSaveCreds } = await useMultiFileAuthState('./temp_auth');
            state = newState;
            saveCreds = async () => {
                await database.saveSession(this.userId, state);
                await newSaveCreds();
